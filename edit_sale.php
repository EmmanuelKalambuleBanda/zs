...
if (isset($_POST['update_sale'])) {
  ...
  if (empty($errors)) {
    ...
    $available_sales_qty = get_available_quantity_sales($p_id);
    $quantity_difference = $s_qty -  $available_sales_qty;

    if ($quantity_difference >= 0) {
      $new_product_qty = $quantity_difference;
    } else {
      $new_product_qty =  $quantity_difference;
    }
    $available_qty = get_available_quantity($p_id);
    if ($s_qty > $available_qty) {
      $session->msg('d', "Invalid quantity update. Sale quantity exceeds available quantity.");
      redirect('edit_sale.php?id=' . (int)$sale['id'], false);
      exit();
    }

    // Check if the product quantity has been manually adjusted
    if ($product['init_quantity'] < $product['quantity'] - $new_product_qty) {
      $session->msg('d', "Product quantity can not exceed intial stock. Further updates are not allowed.");
      redirect('edit_sale.php?id=' . (int)$sale['id'], false);
      exit();
    }
    ...
    if ($result && $db->affected_rows() === 1) {
      update_product_qty($new_product_qty, $p_id);
      $session->msg('s', "Sale updated.");
      redirect('edit_sale.php?id=' . $sale['id'], true);
    } else {
      $session->msg('d', 'No update has been made!');
