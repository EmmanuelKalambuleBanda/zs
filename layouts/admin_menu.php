<!-- Sidebar -->
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
    <!-- Sidebar - Brand -->
    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="admin.php">
        <div class="sidebar-brand-icon rotate-n-15">
            <i class="glyphicon glyphicon-home"></i>
        </div>
        <div class="sidebar-brand-text mx-3">Dashboard</div>
    </a>

    <!-- Divider -->
    <hr class="sidebar-divider my-0">

    <!-- Nav Item - User Management -->
    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseUserManagement"
            aria-expanded="true" aria-controls="collapseUserManagement">
            <i class="glyphicon glyphicon-user"></i>
            <span>User Management</span>
        </a>
        <ul class="collapse" id="collapseUserManagement">
            <li><a class="nav-link" href="group.php">Manage Groups</a></li>
            <li><a class="nav-link" href="users.php">Manage Users</a></li>
        </ul>
    </li>

    <!-- Divider -->
    <hr class="sidebar-divider">

    <!-- Nav Item - Categories -->
    <li class="nav-item">
        <a class="nav-link" href="categorie.php">
            <i class="glyphicon glyphicon-indent-left"></i>
            <span>Categories</span>
        </a>
    </li>

    <!-- Divider -->
    <hr class="sidebar-divider">

    <!-- Nav Item - Products -->
    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseProducts"
            aria-expanded="true" aria-controls="collapseProducts">
            <i class="glyphicon glyphicon-th-large"></i>
            <span>Products</span>
        </a>
        <ul class="collapse" id="collapseProducts">
            <li><a class="nav-link" href="product.php">Manage Products</a></li>
            <li><a class="nav-link" href="add_product.php">Add Products</a></li>
        </ul>
    </li>

    <!-- Divider -->
    <hr class="sidebar-divider">

    <!-- Nav Item - Media Files -->
    <li class="nav-item">
        <a class="nav-link" href="media.php">
            <i class="glyphicon glyphicon-picture"></i>
            <span>Media Files</span>
        </a>
    </li>

    <!-- Divider -->
    <hr class="sidebar-divider">

    <!-- Nav Item - Sales -->
    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseSales"
            aria-expanded="true" aria-controls="collapseSales">
            <i class="glyphicon glyphicon-credit-card"></i>
            <span>POS</span>
        </a>
        <ul class="collapse" id="collapseSales">
            <li><a class="nav-link" href="sales.php">Manage Sales</a></li>
            <li><a class="nav-link" href="add_sale.php">Add Sale</a></li>
        </ul>
    </li>

    <!-- Divider -->
    <hr class="sidebar-divider">

    <!-- Nav Item - Sales Report -->
    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseSalesReport"
            aria-expanded="true" aria-controls="collapseSalesReport">
            <i class="glyphicon glyphicon-duplicate"></i>
            <span>Sales Report</span>
        </a>
        <ul class="collapse" id="collapseSalesReport">
            <li><a class="nav-link" href="sales_report.php">Sales by dates</a></li>
            <li><a class="nav-link" href="monthly_sales.php">Monthly sales</a></li>
            <li><a class="nav-link" href="daily_sales.php">Daily sales</a></li>
        </ul>
    </li>
</ul>
<!-- End of Sidebar -->
``
